 ...
 * ----------------------------------------------------------------------------------------------------------
 * 機能概要：
 * 前年度とのITスキルレベル比較クエリ結果。
 * 今年度と前年度のスキルレベル差分（diff）を明細ごとに保持し、前年比較画面のデータソースとして使用する。
 * ----------------------------------------------------------------------------------------------------------
 * 更新履歴：
 ...
 * @param items             ITスキルごとの比較データ一覧（カスタムスキルは currentLevelValue=null・diff=null で含まれる）
 */
public record ComparisonQueryResult(int inventoryId, String currentFiscalYear, String prevFiscalYear,
     ...
     * @param skillName         スキル名（カスタムスキルの場合はカスタムスキル名）
     * @param currentDetailId   現在の棚卸明細の内部 PK
     * @param currentLevelValue 現在のスキルレベル値（カスタムスキルは採点しないため null）
     * @param currentRemarks    現在の備考
     * @param prevLevelValue    前年度のスキルレベル値（前年度に当該スキルがない場合は null）
     * @param diff              レベル差分（currentLevelValue - prevLevelValue。前年度データなし・カスタムスキルは null）
     */
    public record ComparisonItem(Integer itSkillId, String skillName, int currentDetailId,
                                 Integer currentLevelValue, String currentRemarks,
