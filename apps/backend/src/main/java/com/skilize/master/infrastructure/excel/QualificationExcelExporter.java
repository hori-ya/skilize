...
public class QualificationExcelExporter {
    ...
    private void writeCategories(XSSFWorkbook wb, XSSFCellStyle headerStyle,
        ...
        sheet.setColumnWidth(1, 30 * 256); // カテゴリ名
        sheet.setColumnWidth(2, 10 * 256); // 有効
    ...
    private void writeQualifications(XSSFWorkbook wb, XSSFCellStyle headerStyle,
        ...
        sheet.setColumnWidth(2, 25 * 256); // カテゴリ名 (参考)
        sheet.setColumnWidth(3, 30 * 256); // 資格名
        sheet.setColumnWidth(4, 40 * 256); // 説明
        sheet.setColumnWidth(5, 10 * 256); // 有効

        Row header = sheet.createRow(0);
        setHeader(header, 0, "カテゴリID", headerStyle);
        setHeader(header, 1, "ID", headerStyle);
        setHeader(header, 2, "カテゴリ名", headerStyle); // 参考列ヘッダーもヘッダー色で統一
        ...
        for (Qualification q : qualifications) {
            ...
            setString(row, 2, q.getCategory() != null ? q.getCategory().getName() : "", refStyle); // 参考
