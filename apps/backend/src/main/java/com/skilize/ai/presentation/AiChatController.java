 ...
 * ----------------------------------------------------------------------------------------------------------
 * 機能概要：
 * AIチャット機能のプレゼンテーション層。フロントエンドからのチャットリクエストを受け取り、
 * Python AIサービスに転送して応答を返す。会話履歴はフロントエンドのみで保持する設計。
 * ----------------------------------------------------------------------------------------------------------
 * 更新履歴：
...
public class AiChatController {
    ...
    /**
     * AIチャットエンドポイント。リクエストをコマンドに変換してAiChatServiceに委譲する。
     *
     * @param request チャットリクエスト（メッセージ・モード・会話履歴を含む）
     * @param user    認証済みユーザー（ユーザーIDをAIサービスに渡すために使用）
