...
import { getItSkills, getSkillLevels, getQualifications, getAdSeminars, getSeminarCategories } from '../api/master';
import type { InventoryDetail } from '../types/inventory';
import type { ItSkill, SkillLevel, Qualification, AdSeminar, SeminarCategory } from '../types/master';
...
export default function InventoryPage() {
  ...
  const [seminarCategories, setSeminarCategories] = useState<SeminarCategory[]>([]);
  ...
  useEffect(() => {
    ...
    ]).then(async ([invRes, skillsRes, levelsRes, qualsRes, adsRes, semCatsRes, itDetailsRes, qualDetailsRes, semDetailsRes]) => {
      setInventory(invRes.data);
      setItSkills(skillsRes.data);
      setSkillLevels(levelsRes.data);
      setQualifications(qualsRes.data);
      setAdSeminars(adsRes.data);
      setSeminarCategories(semCatsRes.data);
            ...
            <div className="level-legend">
              {skillLevels.map(lv => (
                <span key={lv.id} className="level-legend-item">
                  <span className="level-legend-lv">Lv{lv.levelValue}</span>
                  {lv.description}
                </span>
              ))}
            </div>
                        ...
                        {skillLevels.map(lv => (
                          <td key={lv.id} className="radio-cell">
                            <input
                              ...
                              checked={row.levelId === lv.id}
                              onChange={() => setCustomSkillRows(prev => prev.map((r, i) =>
                                i === idx ? { ...r, levelId: lv.id } : r))}
                            />
                          </td>
                        ))}
                        <td>
              ...
              <div className="skill-input-panel">
                ...
                {seminarRows.map((row, idx) => (
                  <div key={idx} className={`skill-row${seminarErrors.has(idx) ? ' skill-row--error' : ''}`}>
                    ...
                    <div className="skill-row-body">
                      {!row.isAd && seminarCategories.length > 0 && (
                        <>
                          <label className="form-label">分類</label>
                          <select
                            className="select"
                            value={row.seminarCategoryId ?? ''}
                            onChange={e => setSeminarRows(prev => prev.map((r, i) =>
                              i === idx ? { ...r, seminarCategoryId: e.target.value ? Number(e.target.value) : null } : r))}
                          >
