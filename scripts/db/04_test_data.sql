...
-- =============================================================================
-- 期待コメント（user_expectations）
-- tl_expectation: tl01 が user01・user02 に記入
-- company_expectation: admin が全員に記入
-- =============================================================================

INSERT INTO user_expectations (user_id, tl_expectation, company_expectation) VALUES
(
    (SELECT id FROM users WHERE user_id = 'user01'),
    'SE ロールへの移行を着実に進めており、基本設計書の作成も経験できた成長を高く評価しています。
来年度は要件定義フェーズを担当する機会を積極的に作っていくので、お客様との調整や認識合わせの場でも積極的に発言してほしいと思います。
また、TypeScript 勉強会など自発的な学習姿勢はチームの財産でもあるので、学んだことをチームへ共有・展開してもらえると嬉しいです。
応用情報の取得は昇格にもつながる大切なステップなので、学習計画を一緒に立てながら全力でサポートします。焦らず着実に進めていきましょう。',
    '技術力とコミュニケーション能力をバランスよく伸ばしており、SE としての基盤が整いつつあります。
来年度は要件定義への挑戦や応用情報技術者試験の取得など、一段上のキャリアに向けた取り組みを期待しています。
自主的な学習姿勢を引き続き大切にしながら、チームへの貢献をより広い範囲で発揮してください。中堅エンジニアとしての活躍を期待しています。'
),
(
    (SELECT id FROM users WHERE user_id = 'user02'),
    '前年度に設定したクラウドスキルの習得目標を達成し、着実な成長を見せてくれました。自分から発言する場面も増えてきており、良い変化を感じています。
来年度は応用情報技術者試験の取得と PL/SQL スキルの強化に集中してほしいと思います。試験の 2 か月前からは週次で学習進捗を一緒に確認しますので、困ったことがあればいつでも相談してください。
まだ自分のアイデアや意見を出すことに遠慮がちな面があるように思います。ミーティングや改善提案など、発信の機会を意識的に増やしていくとさらに活躍の場が広がります。',
    '丁寧な仕事ぶりと着実なスキルアップで、チームの安定した戦力として成長しています。
来年度は応用情報技術者試験の取得と DB 周りのスキル強化に取り組み、担当できる業務の幅をさらに広げることを期待しています。
自分の意見をもっと積極的に発信し、チームのなかでの存在感を高めていってください。継続的な成長を応援しています。'
),
(
    (SELECT id FROM users WHERE user_id = 'tl01'),
    NULL,
    'チームリーダーとしてメンバーの育成・プロジェクト管理の両面で高いパフォーマンスを発揮しています。
来年度は PMP 取得とリスク管理スキルの習得を通じて、プロジェクトマネージャーへのステップアップを期待しています。
技術的な専門性とマネジメントスキルを組み合わせ、組織全体の技術力底上げにも貢献していただくことを期待しています。
チームビルディングや後進育成においても引き続きリーダーシップを発揮してください。'
);
...
-- =============================================================================
-- AI分析テストデータ（ai_career_analyses）
-- 表示確認用：user01 の 2024年度 分析結果（COMPLETED）
-- =============================================================================

INSERT INTO ai_career_analyses (user_id, fiscal_year_id, status, analysis_result)
VALUES (
    (SELECT id FROM users WHERE user_id = 'user01'),
    (SELECT id FROM fiscal_years WHERE name = '2024年度'),
    'COMPLETED',
    $json${
      "summary": "プログラミングと SQL を中心に高い実務スキルを保有しており、開発現場の主力として安定した活躍をしています。AWS 認定資格（SAA）の自己学習取得など主体的な学習姿勢も際立っています。今後は SE ロールへのキャリアアップに向けて、基本設計書の作成スキル習得と応用情報技術者試験の取得が重要なステップとなります。",
      "strengths": [
        "詳細設計書を読んだ PG 作成・改修においてレベル4を達成しており、開発現場での即戦力として安定した貢献をしている",
        "SQL の複雑なクエリ（サブクエリ・JOIN）を独力で実務に適用できる高いデータベーススキルを持つ",
        "AWS SAA を自己学習で取得するなど、主体的なスキルアップへの意欲が高く、学習を継続する力がある",
        "バージョン管理（Git）・テスト実施・報告連絡相談など、開発プロセス全体を着実にこなせる総合力がある"
      ],
      "growth_areas": [
        "基本設計書の作成など上流工程スキルの習得（今年度の重点目標として設定済み）",
        "Spring Boot の習熟をレベル3からさらに深め、設計フェーズでも活用できる段階への引き上げ",
        "Python 勉強会で得た知識を実務やチームへ展開する機会の創出"
      ],
      "expectation_fit": "TL・会社ともに SE ロールへのキャリアアップを期待しており、本人の目標（基本設計書スキル・応用情報取得）とも方向性が一致しています。AWS 案件への配置検討がある点からも、技術的な成長の方向性と期待のフィット感は高いです。",
      "recommended_actions": [
        "もし現在担当している案件で詳細設計書に関わる機会があれば、作成側の視点を少し意識してみると基本設計書への理解がより深まるかもしれません",
        "応用情報技術者試験の学習計画をざっくり立ててみて、TL に見せてフィードバックをもらうと安心して進められそうです",
        "Python 勉強会で学んだ内容を、チームへの共有タイムなどで紹介する機会があれば、アウトプットを通じた定着がより進むかもしれません",
        "面談で AWS 案件への参加希望を伝えてみると、クラウドスキルを実務でさらに活かせる機会につながりやすくなるかもしれません"
      ]
    }$json$::jsonb
);
