...
| データ | 方針 |
|--------|------|
| `password_hash` | API レスポンスに含めない。ログ出力禁止 |
| JWT 署名キー | 環境変数のみ。ログ出力禁止 |
| JWT トークン（`Authorization` ヘッダ） | ログ出力禁止 |
| 氏名・メールアドレス | ログ出力禁止（個人情報保護） |
| 面談メモ（`interview_notes`） | `interviewer_id` が一致するユーザー以外には一切返さない。存在有無も含め非開示 |

### 7.7 ログ出力方針

SLF4J + Logback（`@Slf4j`）を使用してログを出力する。詳細は非機能要件定義書「6. アプリケーションログ要件」を参照。
...
`LoggingFilter` がリクエストごとに以下を MDC にセットし、全ログに自動付与する。

| キー | 値 |
|------|----|
| `requestId` | UUID（リクエストごとに払い出し） |
| `userId` | 認証済みユーザーID（未認証時は `"-"`） |

**禁止事項**（詳細は非機能要件 6.4 参照）

- パスワード・パスワードハッシュ・JWTトークン・氏名・メールアドレス等の個人情報はログに出力しない
