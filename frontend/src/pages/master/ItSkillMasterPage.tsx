...
import type { ItSkillCategory, ItSkill } from '../../types/master';
import {
  ...
  return (
    <>
      <div className="master-card__header" style={{ marginBottom: 16 }}>
        ...
        <button className="btn btn--primary btn--sm" onClick={openCreate}>+ 分類追加</button>
      </div>
...
function SkillTab({ skills, categories, onReload }: {
  ...
  const resolveSkillLv1Id = (skill: ItSkill): number | null => skill.category1Id;
  ...
  const openEdit = (s: ItSkill) => {
    // Determine category hierarchy from skill
            ...
            {filteredSkills.length === 0 ? (
              ...
              filteredSkills.map(s => (
                <tr key={s.id}>
                  ...
                  <td><button className="btn btn--secondary btn--sm" onClick={() => openEdit(s)}>編集</button></td>
                </tr>
              ))
            )}
          </tbody>
